#python3, loading mediainfo readings for a media file using MediaInfoDLL3.py
...
def mediainfo_value(stream:int, track:int, param:str, path: Union[Path,str]) -> Union[int,float,str]:
	...
	# it opens and closes MI, unlike mediainfo_value_worker
...
if  len(sys.argv) < 2:
	DIRECTORIES = [ r"D:\\ssTEST\\TEST_VIDS_IMAGES\\2TEST_rotations", ]
else:
	DIRECTORIES = str(sys.argv[1])	# IIRC sys.argv[1] is the script name
#
MI = MediaInfo()
...
for d in DIRECTORIES:
	...
	for path in paths:
		if path.suffix.lower() in vid_extensions:
			...
			print(f'\n--------------------------------------------------------------------------------------------------------\n{tn}. Video File: {path}')
			#
			# The mediainfo approach yields oinly selected info
			#
			MI.Open(str(path))
			mi_dict = {}
			for param in mi_video_params_1:
				#value = mediainfo_value(Stream.Video, track=0, param=param, path=path)	# version: single-value retrieval and lots of open/close
				value = mediainfo_value_worker(Stream.Video, track=0, param=param, path=path)
				mediainfo_value_worker
				mi_dict[param] = value	# any of str, bool, int, float, etc
			MI.Close()
			print(f'\n{tn}. ====================mi_dict=\n{objPrettyPrint.pformat(mi_dict)}')
			#
			# The ffprobe class approach yields all info for the first video stream and first audio stream
			#
			obj_ffprobe = ffprobe(path)
			if obj_ffprobe.return_code != 0:
				print(f'{tn}. ERROR: ffprobe {path} returned error code {obj_ffprobe.return_code} error_code={obj_ffprobe.error_code} error={obj_ffprobe.error}')
			if obj_ffprobe.num_video_streams <= 0:
				print(f'{tn}. ERROR: ffprobe {path} RETURNED VIDEO STREAMS')
			if obj_ffprobe.format_dict is None:
				print(f'{tn}. ERROR: ffprobe VIDEO FORMAT IS None')
			print(f'\n{tn}. ==================== obj_ffprobe=\n{objPrettyPrint.pformat(obj_ffprobe.dict)}')
			#
			#print(f'duration={obj_ffprobe.format_dict.get("duration")}')
			#print(f'bit_rate={obj_ffprobe.format_dict.get("bit_rate")}')
			#print(f'width/height/dar ff={obj_ffprobe.first_video.get("width")} x {obj_ffprobe.first_video.get("height")} ### ff_dar={obj_ffprobe.first_video.get("display_aspect_ratio")} ... mi={mi_dict.get("width")} x {mi_dict.get("height")} ### mi_dar={mi_dict.get("DisplayAspectRatio")}')
			#print(f'ff_codec_name={obj_ffprobe.first_video.get("codec_name")} ... mi_Format={mi_dict.get("Format")}')
			#print(f'ff_nb_frames={obj_ffprobe.first_video.get("nb_frames")} ... mi_FrameCount={mi_dict.get("FrameCount")}')
			#print(f'ff_rotation={obj_ffprobe.first_video.get("rotation")} ... mi_Rotation={mi_dict.get("Rotation")}')
			#print(f'ff_r_frame_rate={obj_ffprobe.first_video.get("r_frame_rate")} ({eval(obj_ffprobe.first_video.get("r_frame_rate")) if obj_ffprobe.first_video.get("r_frame_rate") is not None else None}) ... FrameRate_Mode={mi_dict.get("FrameRate_Mode")} ### mi_FrameRate_Num={mi_dict.get("FrameRate_Num")} ### mi_FrameRate_Den={mi_dict.get("FrameRate_Den")} ### mi_FrameRate={mi_dict.get("FrameRate")} ### ')
			#print(f'ff_avg_frame_rate={obj_ffprobe.first_video.get("avg_frame_rate")} ### ({eval(obj_ffprobe.first_video.get("avg_frame_rate")) if obj_ffprobe.first_video.get("avg_frame_rate") is not None else None}) ... mi_FrameRate_Mode={mi_dict.get("FrameRate_Mode")} ### mi_FrameRate_Minimum={mi_dict.get("FrameRate_Minimum")} ### mi_FrameRate_Nominal={mi_dict.get("FrameRate_Nominal")} ### mi_FrameRate_Maximum={mi_dict.get("FrameRate_Maximum")} ### mi_FrameRate_Real={mi_dict.get("FrameRate_Real")}')
			#print(f'ff_chroma_location={obj_ffprobe.first_video.get("chroma_location")} ... mi_ChromaSubsampling={mi_dict.get("ChromaSubsampling")}')
			#print(f'ff_color_space={obj_ffprobe.first_video.get("color_space")} ... mi_ColorSpace={mi_dict.get("ColorSpace")} ### mi_Standard={mi_dict.get("Standard")}')
			#print(f'ff_color_matrix={obj_ffprobe.first_video.get("color_matrix")} ### ff_matrix={obj_ffprobe.first_video.get("displaymatrix")} ... mi_matrix_coefficients={mi_dict.get("matrix_coefficients")} ### mi_Standard={mi_dict.get("Standard")}')
			#print(f'ff_color_transfer={obj_ffprobe.first_video.get("color_transfer")} ... mi_transfer_characteristics={mi_dict.get("transfer_characteristics")} ### mi_Standard={mi_dict.get("Standard")} ... ff_matrix={obj_ffprobe.first_video.get("displaymatrix")}')
			#print(f'ff_color_primaries={obj_ffprobe.first_video.get("color_primaries")} ... mi_colour_primaries={mi_dict.get("colour_primaries")} ### mi_Standard={mi_dict.get("Standard")} ... ff_matrix={obj_ffprobe.first_video.get("displaymatrix")}')
			#print(f'ff_color_range={obj_ffprobe.first_video.get("color_range")} ... mi_colour_range={mi_dict.get("colour_range")} ### mi_Standard={mi_dict.get("Standard")} ... ff_matrix={obj_ffprobe.first_video.get("displaymatrix")}')
			#print(f'ff_field_order={obj_ffprobe.first_video.get("field_order")} ... mi_ScanType={mi_dict.get("ScanType")} ### mi_ScanOrder={mi_dict.get("ScanOrder")} ### mi_ScanOrder_Stored={mi_dict.get("ScanOrder_Stored")} ### mi_ScanOrder_StoredDisplayedInverted={mi_dict.get("ScanOrder_StoredDisplayedInverted")} ### mi_Standard={mi_dict.get("Standard")}')
			#print(f'ff_space={obj_ffprobe.first_video.get("color_space")} ### ff_matrix={obj_ffprobe.first_video.get("color_matrix")} ### ff_transfer={obj_ffprobe.first_video.get("color_transfer")} ### ff_primaries={obj_ffprobe.first_video.get("color_primaries")} ### ff_range={obj_ffprobe.first_video.get("color_range")} ### ff_matrix={obj_ffprobe.first_video.get("displaymatrix")}')
			del obj_ffprobe
	print(f'\nFinished. {n} files queried using mediainfo and ffprobe in "{d}".\n')
print(f'\nFinished. {tn} files queried using mediainfo and ffprobe in total.\n')
del MI
sys.exit()
