...
def garbage_collect():
	...
	try:
		...
		raise_ValueError_wrapper(f'vs_transitions: garbage_collect: ERROR: unknown error occurred at high level try/except:\n{e}\n{type(e)}\n{str(e)}')

def check_clips_preStack_and_abort(ID: int, list_of_clips:list, abort=False):
	...
	try:
		...
		for i in range(1,length_list_of_clips):	# base 0, so 1 starts at second list entry
			if list_of_clips[i].width != W0 or list_of_clips[0].height != H0:
				if abort:
					...
					raise_ValueError_wrapper(f'vs_transitions: check_clips_preStack_and_abort: ID={ID}: ERROR: unmatched clip dimensions clip[0] width/height={W0}/{H0} clip[{i}] width/height={list_of_clips[i].width}/{list_of_clips[i].height}\nclip[0]={list_of_clips[0]}\nclip[{i}={list_of_clips[i]}')
				else:
			...
			if list_of_clips[i].format != F0:
				if abort:
					...
					raise_ValueError_wrapper(f'vs_transitions: check_clips_preStack_and_abort: ID={ID}: ERROR: inconsistent clip formats clip[0] format={F0} clip[{i}] format={list_of_clips[i].format}\nclip[0]={list_of_clips[0]}\nclip[{i}={list_of_clips[i]}')
				else:
			...
			if len(list_of_clips[i]) != L0:
				if abort:
					...
					raise_ValueError_wrapper(f'vs_transitions: check_clips_preStack_and_abort: ID={ID}: ERROR: inconsistent clip lengths clip[0] length={L0} clip[{i}] length={len(list_of_clips[i])}\nclip[0]={list_of_clips[0]}\nclip[{i}={list_of_clips[i]}')
				else:
		...
		raise_ValueError_wrapper(f'vs_transitions: check_clips_preStack_and_abort: ID={ID}: ERROR: unknown error occurred at high level try/except:\n{e}\n{type(e)}\n{str(e)}')

def print_stack_clips_list_properties(ID: int, caller: Callable, list_of_clips:list, debug=False):
	...
	for i in range(0,length_list_of_clips):
		try:
			...
			raise_ValueError_wrapper(f'vs_transitions: print_stack_clips_list_properties: EXCEPTION: {caller.__name__}: ID={ID}: ERROR: failed to find/print characteristics clip[{i}] of {length_list_of_clips-1} (base 0) \n{e}\n{type(e)}\n{str(e)}')
	return

def StackHorizontal_wrapper(ID: int, list_of_clips:list) -> vs.VideoNode:
	...
	###try:
	###	check_clips_preStack_and_abort(ID, list_of_clips, abort=False)	### THIS IS FOR ONLY DURING DEVELOPMENT
	###	try:
	###		c = core.std.StackHorizontal(list_of_clips)	# the real StackHorizontal
	###	except Exception as e:
	###		print_stack_clips_list_properties(ID, StackHorizontal_wrapper, list_of_clips, debug=False)
	###		raise_ValueError_wrapper(f'vs_transitions: StackHorizontal_wrapper: ERROR: StackHorizontal_wrapper ID={ID} error:\n{e}\n{type(e)}\n{str(e)}')
	###	if not isinstance(c, vs.VideoNode):
	###		print_stack_clips_list_properties(ID, StackHorizontal_wrapper, list_of_clips, debug=False)
	###		raise_ValueError_wrapper(f'vs_transitions: StackHorizontal_wrapper: ERROR: StackHorizontal_wrapper ID={ID} did not return a type vs.VideoNode video clip')
	###	elif len(c) <= 0:
	###		print_stack_clips_list_properties(ID, StackHorizontal_wrapper, list_of_clips, debug=False)
	###		raise_ValueError_wrapper(f'vs_transitions: StackHorizontal_wrapper: ERROR: StackHorizontal_wrapper ID={ID} returned a zero length video clip')
	###	return c
	###except Exception as e:
	###	print_stack_clips_list_properties(ID, StackHorizontal_wrapper, list_of_clips, debug=False)
	###	raise_ValueError_wrapper(f'vs_transitions: StackHorizontal_wrapper: ID={ID} ERROR: unknown error occurred at high level try/except:\n{e}\n{type(e)}\n{str(e)}')
	
def StackVertical_wrapper(ID: int, list_of_clips:list) -> vs.VideoNode:
	...
	###		c = core.std.StackVertical(list_of_clips)	# the real StackVertical
	###	except Exception as e:
	###		print_stack_clips_list_properties(ID, StackVertical_wrapper, list_of_clips, debug=False)
	###		raise_ValueError_wrapper(f'vs_transitions: StackVertical_wrapper: ERROR: StackVertical_wrapper ID={ID} error:\n{e}\n{type(e)}\n{str(e)}')
	###	if not isinstance(c, vs.VideoNode):
	###		print_stack_clips_list_properties(ID, StackVertical_wrapper, list_of_clips, debug=False)
	###		raise_ValueError_wrapper(f'vs_transitions: StackVertical_wrapper: ERROR: StackVertical_wrapper ID={ID} did not return a type vs.VideoNode video clip')
	###	elif len(c) <= 0:
	###		print_stack_clips_list_properties(ID, StackVertical_wrapper, list_of_clips, debug=False)
	###		raise_ValueError_wrapper(f'vs_transitions: StackVertical_wrapper: ERROR: StackVertical_wrapper ID={ID} returned a zero length video clip')
	###	return c
	###except Exception as e:
	###	print_stack_clips_list_properties(ID, StackVertical_wrapper, list_of_clips, debug=False)
	###	raise_ValueError_wrapper(f'vs_transitions: StackVertical_wrapper: ID={ID} ERROR: unknown error occurred at high level try/except:\n{e}\n{type(e)}\n{str(e)}')

##################################################################################################################################################################################################################################################################################################

##################################################################################################################################################################################################################################################################################################
def _check_clips(ID: int, frames: int, caller: Callable, *clips: vs.VideoNode, **kwargs) -> None:
	...
	if frames <= 0:
		raise_ValueError_wrapper(f"{caller.__name__}: ID={ID} `frames` cannot be less than 1")
	same_check = set()
	for clip in clips:
		if clip.format is None:
			raise_ValueError_wrapper(f"{caller.__name__}: ID={ID} all clips must be constant-format")
		if 0 in (clip.width, clip.height):
			raise_ValueError_wrapper(f"{caller.__name__}: ID={ID} all clips must be constant-resolution")
		if clip.num_frames < frames:
			raise_ValueError_wrapper(f"{caller.__name__}: ID={ID} all clips must have at least {frames} frames")
		...
		if kwargs:
			if ("subsampling" in kwargs) and kwargs["subsampling"]:
				if clip.format.subsampling_w != 0 or clip.format.subsampling_h != 0:
					raise_ValueError_wrapper(
						f"{caller.__name__}: ID={ID} all clips must have 444 chroma subsampling for a non-mod2 resize"
					)

	if len(same_check) > 1:
		raise_ValueError_wrapper(f"{caller.__name__}: ID={ID} all clips must be same format and resolution")
		pass
...
def add_together(ID: int, clipa: vs.VideoNode, clipb: vs.VideoNode) -> vs.VideoNode:
	...
	_check_clips(ID, frames_, add_together, clipa, clipb)
	return clipa + clipb


def fade(ID: int, clipa: vs.VideoNode, clipb: vs.VideoNode, frames: Optional[int] = None) -> vs.VideoNode:
	"""Cross-fade clips."""
	...
	_check_clips(ID, frames_, fade, clipa, clipb)
...
def poly_fade(ID: int, 
	...
	if not (1 <= exponent <= 5):
		raise_ValueError_wrapper("poly_fade: exponent must be an int between 1 and 5 (inclusive)")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, fade, clipa, clipb)
...
def fade_to_black(ID: int, src_clip: vs.VideoNode, frames: Optional[int] = None) -> vs.VideoNode:
	...
	if src_clip.format is None:
		raise_ValueError_wrapper("fade_to_black: `src_clip` must be a constant format VideoNode")
...
def fade_from_black(ID: int, src_clip: vs.VideoNode, frames: Optional[int] = None) -> vs.VideoNode:
...
def wipe(ID: int, 
	...
	if direction not in [Direction.LEFT, Direction.RIGHT, Direction.UP, Direction.DOWN]:
		raise_ValueError_wrapper("wipe: give a proper direction")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, wipe, clipa, clipb)
	...
	if direction in [Direction.LEFT, Direction.RIGHT]:
		stack = StackHorizontal_wrapper(ID, [black_clip, mask_horiz, white_clip])
		w = mask_horiz.width
		...
		stack = StackVertical_wrapper(ID, [black_clip, mask_vert, white_clip])
		h = mask_vert.height
...
def cube_rotate(ID: int, 
	...
	if direction not in [Direction.LEFT, Direction.RIGHT, Direction.UP, Direction.DOWN]:
		raise_ValueError_wrapper("cube_rotate: give a proper direction")
	if not (0 <= exaggeration <= 100):
		raise_ValueError_wrapper(f"cube_rotate: exaggeration {exaggeration} not between 0 and 100")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, cube_rotate, clipa, clipb)
	...
	if direction in [Direction.LEFT, Direction.RIGHT]:
		...
		def _rotate(n: int):
			...
			if w_dec == clipa.width:
				...
				if direction == Direction.LEFT:
					return StackHorizontal_wrapper(ID, [clipa_squeezed, clipb_squeezed])
				elif direction == Direction.RIGHT:
					return StackHorizontal_wrapper(ID, [clipb_squeezed, clipa_squeezed])
		...
		def _rotate(n: int):
			...
			if h_dec == clipa.height:
				...
				if direction == Direction.UP:
					return StackVertical_wrapper(ID, [clipa_squeezed, clipb_squeezed])
				elif direction == Direction.DOWN:
					return StackVertical_wrapper(ID, [clipb_squeezed, clipa_squeezed])
...
def linear_boundary(ID: int, 
	...
	if clipa_movement not in [MiscConstants.SLIDE, MiscConstants.SQUEEZE]:
		raise_ValueError_wrapper("linear_boundary: clipa_movement must be either a slide or a squeeze")
	if clipb_movement not in [MiscConstants.SLIDE, MiscConstants.EXPAND]:
		raise_ValueError_wrapper("linear_boundary: clipb_movement must be either a slide or an expand")
	if direction not in [Direction.LEFT, Direction.RIGHT, Direction.UP, Direction.DOWN]:
		raise_ValueError_wrapper("linear_boundary: give a proper direction")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	...
	_check_clips(ID, frames_, linear_boundary, clipa, clipb, subsampling=check_for_subsampling)
	...
	if clipa_movement == clipb_movement == MiscConstants.SLIDE:
		...
		def _stack(clipa_: vs.VideoNode, clipb_: vs.VideoNode) -> vs.VideoNode:
			...
			if direction == Direction.LEFT:
				return StackHorizontal_wrapper(ID, [clipa_, clipb_])
			elif direction == Direction.RIGHT:
				return StackHorizontal_wrapper(ID, [clipb_, clipa_])
			elif direction == Direction.UP:
				return StackVertical_wrapper(ID, [clipa_, clipb_])
			elif direction == Direction.DOWN:
				return StackVertical_wrapper(ID, [clipb_, clipa_])
		...
		def _slide_expand(n: int):
			...
			if direction in [Direction.LEFT, Direction.RIGHT]:
				...
				if direction == Direction.LEFT:
					stack = StackHorizontal_wrapper(ID, [clipa_t_zone, clipb_t_zone.resize.Spline36(width=w)])
					return stack.std.Crop(left=w)
				elif direction == Direction.RIGHT:
					stack = StackHorizontal_wrapper(ID, [clipb_t_zone.resize.Spline36(width=w), clipa_t_zone])
				...
				if direction == Direction.UP:
					stack = StackVertical_wrapper(ID, [clipa_t_zone, clipb_t_zone.resize.Spline36(height=h)])
					return stack.std.Crop(top=h)
				elif direction == Direction.DOWN:
					stack = StackVertical_wrapper(ID, [clipb_t_zone.resize.Spline36(height=h), clipa_t_zone])
		...
		def _squeeze_slide(n: int):
			...
			if direction in [Direction.LEFT, Direction.RIGHT]:
				...
				if direction == Direction.LEFT:
					stack = StackHorizontal_wrapper(ID, [clipa_t_zone.resize.Spline36(width=w), clipb_t_zone])
					return stack.std.Crop(right=w)
				elif direction == Direction.RIGHT:
					stack = StackHorizontal_wrapper(ID, [clipb_t_zone, clipa_t_zone.resize.Spline36(width=w)])
					return stack.std.Crop(left=w)
				...
				if direction == Direction.UP:
					stack = StackVertical_wrapper(ID, [clipa_t_zone.resize.Spline36(height=h), clipb_t_zone])
					return stack.std.Crop(bottom=h)

				elif direction == Direction.DOWN:
					stack = StackVertical_wrapper(ID, [clipb_t_zone, clipa_t_zone.resize.Spline36(height=h)])
					return stack.std.Crop(top=h)
		...
		def _squeeze_expand(n: int) -> vs.VideoNode:
			...
			if direction in [Direction.LEFT, Direction.RIGHT]:
				...
				if direction == Direction.LEFT:
					return StackHorizontal_wrapper(ID, 
...
def push(ID: int, clipa: vs.VideoNode, clipb: vs.VideoNode, frames: Optional[int] = None, direction: Direction = Direction.LEFT):
	...
	return linear_boundary(ID, clipa, clipb, MiscConstants.SLIDE, MiscConstants.SLIDE, frames=frames, direction=direction)


def slide_expand(ID: int, 
	...
	return linear_boundary(ID, clipa, clipb, MiscConstants.SLIDE, MiscConstants.EXPAND, frames=frames, direction=direction)


def squeeze_slide(ID: int, 
	clipa: vs.VideoNode, clipb: vs.VideoNode, frames: Optional[int] = None, direction: Direction = Direction.LEFT
):
	...
	return linear_boundary(ID, clipa, clipb, MiscConstants.SQUEEZE, MiscConstants.SLIDE, frames=frames, direction=direction)


def squeeze_expand(ID: int, 
	clipa: vs.VideoNode, clipb: vs.VideoNode, frames: Optional[int] = None, direction: Direction = Direction.LEFT
):
...
def cover(ID: int, 
	...
	if direction not in [Direction.LEFT, Direction.RIGHT, Direction.UP, Direction.DOWN]:
		raise_ValueError_wrapper("cover: give a proper direction")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, cover, clipa, clipb)
	clipa_clean, clipb_clean, clipa_t_zone, clipb_t_zone = _transition_clips(clipa, clipb, frames_)
	...
	def _cover(n: int) -> vs.VideoNode:
		...
		if direction == Direction.LEFT:
			...
			stack = StackHorizontal_wrapper(ID, [cropped_a, clipb_t_zone])
			return stack.resize.Spline36(width=clipa.width, src_width=clipa.width)
		elif direction == Direction.RIGHT:
			cropped_a = clipa_t_zone.std.Crop(left=w)
			stack = StackHorizontal_wrapper(ID, [clipb_t_zone, cropped_a])
...
def reveal(ID: int, 
	...
	if direction not in [Direction.LEFT, Direction.RIGHT, Direction.UP, Direction.DOWN]:
		raise_ValueError_wrapper("reveal: give a proper direction")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, reveal, clipa, clipb)
	clipa_clean, clipb_clean, clipa_t_zone, clipb_t_zone = _transition_clips(clipa, clipb, frames_)
	...
	def _reveal(n: int) -> vs.VideoNode:
		...
		if direction == Direction.LEFT:
			...
			stack = StackHorizontal_wrapper(ID, [clipa_t_zone, cropped_b])
			...
			stack = StackHorizontal_wrapper(ID, [cropped_b, clipa_t_zone])
			return stack.resize.Spline36(width=clipa.width, src_width=clipa.width)
		elif direction == Direction.UP:
			cropped_b = clipb_t_zone.std.Crop(top=h)
			stack = StackVertical_wrapper(ID, [clipa_t_zone, cropped_b])
			...
			stack = StackVertical_wrapper(ID, [cropped_b, clipa_t_zone])
			return stack.resize.Spline36(height=clipa.height, src_height=clipa.height)
...
def curtain_cover(ID: int, 
	...
	if axis not in [Direction.HORIZONTAL, Direction.VERTICAL]:
		raise_ValueError_wrapper("curtain_cover: give a proper axis")
	if axis == Direction.HORIZONTAL and clipa.width % 2:
		raise_ValueError_wrapper("curtain_cover: for horizontal reveal, input clips must have an even width")
	elif axis == Direction.VERTICAL and clipa.height % 2:
		raise_ValueError_wrapper("curtain_cover: for vertical reveal, input clips must have an even height")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, curtain_cover, clipa, clipb)
	clipa_clean, clipb_clean, clipa_t_zone, clipb_t_zone = _transition_clips(clipa, clipb, frames_)
	...
	def _curtain_cover(n: int) -> vs.VideoNode:
		...
		if axis == Direction.HORIZONTAL:
			...
			return StackHorizontal_wrapper(ID, [clipb_left, clipa_cropped, clipb_right])
			...
			return StackVertical_wrapper(ID, [clipb_top, clipa_cropped, clipb_bottom])
...
def curtain_reveal(ID: int, 
	...
	if axis not in [Direction.HORIZONTAL, Direction.VERTICAL]:
		raise_ValueError_wrapper("curtain_reveal: give a proper axis")
	if axis == Direction.HORIZONTAL and clipa.width % 2:
		raise_ValueError_wrapper("curtain_reveal: for horizontal reveal, input clips must have an even width")
	elif axis == Direction.VERTICAL and clipa.height % 2:
		raise_ValueError_wrapper("curtain_reveal: for vertical reveal, input clips must have an even height")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, curtain_reveal, clipa, clipb)
	clipa_clean, clipb_clean, clipa_t_zone, clipb_t_zone = _transition_clips(clipa, clipb, frames_)
	...
	def _curtain_reveal(n: int) -> vs.VideoNode:
		...
		if axis == Direction.HORIZONTAL:
			...
			return StackHorizontal_wrapper(ID, [clipa_left, clipb_cropped, clipa_right])
			...
			return StackVertical_wrapper(ID, [clipa_top, clipb_cropped, clipa_bottom])
...
def pixellate(ID: int, 
	...
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if frames_ < 4:
		raise_ValueError_wrapper("pixellate: transition must be at least 4 frames long")
	...
	if lowest_target_w < 1 or lowest_target_w > clipa.width:
		raise_ValueError_wrapper("pixellate: `lowest_target_w` must be at least one and at most the width of the source clips")
	if lowest_target_h < 1 or lowest_target_h > clipa.height:
		raise_ValueError_wrapper("pixellate: `lowest_target_h` must be at least one and at most the height of the source clips")
	if lowest_target_w == clipa.width and lowest_target_h == clipa.height:
		raise_ValueError_wrapper("pixellate: at least one target dimension must be lower than the source dimensions")
	...
	_check_clips(ID, frames_, pixellate, clipa, clipb)
	clipa_clean, clipb_clean, clipa_t_zone, clipb_t_zone = _transition_clips(clipa, clipb, frames_)
...
def peel(ID: int, 
	...
	if direction not in [Direction.LEFT, Direction.RIGHT, Direction.UP, Direction.DOWN]:
		raise_ValueError_wrapper("peel: give a proper direction")
	frames_ = frames or min(clipa.num_frames, clipb.num_frames)
	if TYPE_CHECKING:
		assert isinstance(frames_, int)
	_check_clips(ID, frames_, peel, clipa, clipb, subsampling=True)
	clipa_clean, clipb_clean, clipa_t_zone, clipb_t_zone = _transition_clips(clipa, clipb, frames_)
			...
			if direction in [Direction.LEFT, Direction.RIGHT]:
				...
				if clipb_w == 0:
					...
					if direction == Direction.LEFT:
						...
						return StackHorizontal_wrapper(ID, [clipa_cropped, clipb_cropped])
					elif direction == Direction.RIGHT:
						clipa_cropped = clipa_t_zone.std.Crop(left=clipb_w)
						clipb_cropped = clipb_t_zone.std.Crop(right=clipa_w)
						return StackHorizontal_wrapper(ID, [clipb_cropped, clipa_cropped])
			elif direction in [Direction.UP, Direction.DOWN]:
				...
				if clipb_h == 0:
					...
					if direction == Direction.UP:
						...
						return StackVertical_wrapper(ID, [clipa_cropped, clipb_cropped])
					elif direction == Direction.DOWN:
						clipa_cropped = clipa_t_zone.std.Crop(top=clipb_h)
						clipb_cropped = clipb_t_zone.std.Crop(bottom=clipa_h)
						return StackVertical_wrapper(ID, [clipb_cropped, clipa_cropped])
...
def round_to(f: Fraction, n: int) -> int:
	...
	if n < 1:
		raise_ValueError_wrapper("round_to: `n` must be an integer greater than 0")
	if n == 1:
