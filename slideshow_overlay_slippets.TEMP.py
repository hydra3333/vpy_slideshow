...
def generate_silence_padding(duration, silent_audio_path):
	if DEBUG: print(f"generate_silence_padding: generate silent padding: silent_audio_path='{silent_audio_path}'",flush=True,file=sys.stderr)
	ffmpeg_cmd = [
		FFMPEG_PATH,
		'-hide_banner',
		'-v', 'warning',
	...
	try:
		if DEBUG: print(f"generate_silence_padding: ffmpeg_cmd='{ffmpeg_cmd}'",flush=True)
		subprocess.run(ffmpeg_cmd, check=True)
	except subprocess.CalledProcessError as e:
		print(f"generate_silence_padding: Error: subprocess.CalledProcessError Failed to transcode audio:\n{ffmpeg_cmd}\n{str(e)}",flush=True,file=sys.stderr)
		sys.exit(1)
	except Exception as e:
		print(f"generate_silence_padding: Unexpected error: \n{ffmpeg_cmd}\n{str(e)}",flush=True,file=sys.stderr)
		sys.exit(1)
...
if __name__ == "__main__":
	...
	print(f"Background audio (or None for silence)                  : {background_audio_path}",flush=True)
